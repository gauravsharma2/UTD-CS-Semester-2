...
def tokenize(values):
    ...
    sgml_values = re.sub(r'[^a-zA-Z0-9.\s]', ' ', sgml_values)
    # Remove all dots, e.g. U.S.A. becomes USA
    sgml_values = re.sub(r'[^a-zA-Z0-9\s]', '', sgml_values)
...
def get_w1_weight(tf, max_tf, collection_size, df):
    ...
    return (0.4 + 0.6 * math.log10(float(tf) + 0.5) / math.log10(float(max_tf) + 1.0)) * \
           (math.log10(float(collection_size) / float(df)) /
            math.log10(float(collection_size)))
...
def normalize_weights(curr_map):
    ...
    for key in curr_map:
        ...
        for value in values:
            colon_split = value.split(":")
            normalized_weight_value = round(
                float(colon_split[1]) / sqrt_sq_sum, 3)
...
def generate_weight_vector_map(index, collection_size, avg_doclen):
    ...
    for key in index:
        ...
        for posting in postings_list:
            ...
            w2_weight = get_w2_weight(
                tf, doclen, avg_doclen, collection_size, df)
...
def get_top5_documents(query_weight_vector, document_weight_vector):
    ...
    for key_q in query_weight_vector:
        ...
        # This map stores cosine products of a given query with all documents in collection
        cos_prod_map = {}
        ...
        for key_d in document_weight_vector:
            ...
            for term in document_map:
                if term in query_map:
                    cos_prod += round(float(document_map[term])
                                      * float(query_map[term]), 3)

            # This map is of the form cos_prod: document weight vector representation
            cos_prod_map.update(
                {cos_prod: str(key_d) + " " + str(document_weight_vector[key_d])+", " })
        ...
        for key in cos_prod_map:
            ...
            print("Rank:", i, " Score:", round(key, 2),
...
# Change to point to the respective directory
directory = "/people/cs/s/sanda/cs6322/Cranfield/*"
...
for file in collection:
    ...
    for values in root.findall('*'):
        ...
        for token in token_list:
            if token not in stopwords:
                # Append every split word to the list of tokens
                tokens.append(token)
        doclen += len(tokens)
    ...
    index_unsorted = get_unsorted_index(
        index_unsorted, lemma_list, doc_id_counter, doclen)
...
document_weight_vector_1, document_weight_vector_2 = generate_weight_vector_map(
    index, collection_size, avg_doclen)
